...
def load_maps():
    ...
    maps["variant_opening_stock"] = {
        int(k): v for k, v in raw["variant_opening_stock"].items()
    }
...
def build_state(maps, rng):
    ...
    lambda_arr = np.array(
        [
            SEG_LAMBDA.get(maps["customer_segments"].get(int(c), "occasional"), 0.07)
            for c in cids
        ],
        dtype=np.float64,
    )
    join_arr = np.array(
        [maps["customer_join_day"].get(int(c), 0) for c in cids], dtype=np.int32
    )
    churn_arr = np.array(
        [maps["churn_days"].get(int(c), 9999) for c in cids], dtype=np.int32
    )
    tc_start_arr = np.array(
        [maps["temp_churn"].get(int(c), [-1, -1])[0] for c in cids], dtype=np.int32
    )
    tc_end_arr = np.array(
        [maps["temp_churn"].get(int(c), [-1, -1])[1] for c in cids], dtype=np.int32
    )
    is_credit_arr = np.array(
        [1 if int(c) in credit_set else 0 for c in cids], dtype=np.int8
    )

    # At-risk customers: lambda decays over simulation
    at_risk_mask = np.array(
        [
            1 if maps["customer_segments"].get(int(c), "") == "at_risk" else 0
            for c in cids
        ],
        dtype=np.int8,
    )
...
def build_basket(cid, day_idx, current_date, maps, rng):
    ...
    for kw_a, kw_b, prob in STRONG_COMBOS:
        a_match = any(kw_a.lower() in bp for bp in basket_products) or any(
            kw_a.lower() in bc for bc in basket_cats_lower
        )
        if a_match:
            if rng.random() < prob:
                partner_vids = []
...
def handle_payment(cid, total_bill, current_date, maps, state, rng, payments_batch):
    ...
    t = current_date
    dstr = (
        f"{t.strftime('%Y-%m-%d')} {rng.integers(8,21):02d}:{rng.integers(0,60):02d}:00"
    )
...
def handle_spontaneous_payments(
    day_idx, current_date, maps, state, rng, payments_batch
):
    """
...
# ═══════════════════════════════════════════════════════════════════════════
def group_restocks_into_invoices(
    restock_needs, current_date, state, maps, rng, invoices_batch, p_items_batch
):
    """
    ...
    for sid, items in sup_batches.items():
        ...
        invoices_batch.append(
            (inv_id, sid, inv_dstr, round(float(total_amt), 2), ref_no)
        )
...
def main():
    ...
    print(
        f"   Simulation: {START_DATE.strftime('%Y-%m-%d')} → {(START_DATE + timedelta(days=TOTAL_DAYS-1)).strftime('%Y-%m-%d')}"
    )
    print(
        f"   Customers : {n_cust:,}  |  Variants: {n_vids:,}  |  Days: {TOTAL_DAYS:,}"
    )
    ...
    for day_idx in range(TOTAL_DAYS):
        ...
        for idx in visitor_idxs:
            ...
            sale_dt = current_date.replace(
                hour=sale_hour, minute=sale_min, second=sale_sec
            )
    ...
    balance_updates = [
        (round(max(0.0, b), 2), cid) for cid, b in state["cust_debt"].items()
    ]
