...
CATALOG = {
    ...
    "Cleaning": {
        ...
        "lines": [
            ...
            "Washing Powder Quick Wash",
            "Matic Liquid Detergent",
            "Washing Bar Hard",
            "Fabric Conditioner Jasmine",
            "Fabric Conditioner Rose",
            "Dishwash Bar Lemon",
            "Dishwash Gel Lemon",
            "Dishwash Powder",
            "Floor Cleaner Pine",
            ...
            "Toilet Cleaner Blue",
            "Toilet Cleaner Thick",
            "Glass Cleaner Spray",
            "Bathroom Cleaner",
            "Kitchen Degreaser",
            "Mosquito Coil Green",
            "Mosquito Liquid Refill",
            "Mosquito Spray Instant",
            "Cockroach Spray",
        ],
        "sizes": [
            ("200g Pack", 28.0, 0.5),
    ...
    "Baby Care": {
        ...
        "lines": [
            ...
            "Diaper Pants M Size",
            "Diaper Pants L Size",
            "Diaper Pants XL Size",
            "Rash Cream Protective",
            "Baby Wash 2in1",
            "Baby Cereal Rice Stage1",
            "Baby Cereal Wheat Stage2",
        ],
        "sizes": [
            ("100g", 90.0, 1.0),
            ("200ml", 130.0, 1.0),
    ...
    "Health OTC": {
        ...
        "lines": [
            ...
            "Chyawanprash Original",
            "Chyawanprash Special",
            "Digestive Tablets",
            "Antacid Gel",
            "Pain Relief Ointment Strong",
            "Cold Balm Original",
            "Muscle Relief Spray",
            "Glucose Powder Orange",
            "Glucose Powder Lemon",
    ...
    "Stationery": {
        ...
        "lines": [
            ...
            "Pencil Sharpener",
            "30cm Ruler Scale",
            "Notebook 100 Pages Ruled",
            "Notebook 200 Pages Ruled",
            "Drawing Book A4",
            "Crayon Set 12 Colours",
            "Sketch Pen Set 24",
            "Highlighter Yellow",
        ],
        "sizes": [
    ...
    "Pet Food": {
        ...
        "lines": [
            "Adult Dog Chicken Rice",
            "Adult Dog Vegetarian",
            "Puppy Food Chicken",
            "Senior Dog Chicken",
            "Dog Treat Biscuit Milk",
            "Dog Treat Chicken Strip",
            "Adult Cat Fish Ocean",
            "Adult Cat Chicken",
            "Kitten Growth Formula",
            "Cat Treat Temptations",
            "Fish Food Tropical",
        ],
        "sizes": [
            ("500g Pack", 200.0, 1.0),
...
CITY_AREAS = [
    ...
    "Steel Plant Area Visakhapatnam",
    "Durgapur WB",
    "Asansol WB",
    "Bokaro Steel City",
    "Raipur CG",
...
def generate_products(conn, rng):
    ...
    for cat_name, cat in CATALOG.items():
        ...
        for brand, line in combos:
            ...
            is_fast = any(
                kw in prod_name.lower() for kw in cat.get("fast_mover_keywords", [])
            )
    ...
    c.executemany(
        "INSERT OR IGNORE INTO product (name, category) VALUES (?, ?)", product_rows
    )
    conn.commit()
...
def build_and_save_maps(conn, customer_rows, supplier_rows, rng):
    ...
    variant_reorder_point = {str(r[0]): max(5.0, round(r[2] * 0.18, 1)) for r in vdata}
    ...
    for sid, sname in supplier_rows:
        ...
        if not cats:
            ...
            cats = [
                str(all_cats[i])
                for i in rng.choice(len(all_cats), size=n, replace=False)
            ]
    ...
    seg_labels = (
        ["loyal"] * 40 + ["credit"] * 25 + ["occasional"] * 25 + ["at_risk"] * 10
    ) * (n_cust // 100 + 1)
    ...
    for pa, pb in COMBO_PAIRS_NAMES:
        ...
        if not va:
            va = [
                v
                for pn, vlist in product_to_variants.items()
                if pa.lower() in pn.lower()
                for v in vlist
            ]
        if not vb:
            vb = [
    ...
    meta = {
        ...
        "total_variants": c.execute("SELECT COUNT(*) FROM product_variant").fetchone()[
            0
        ],
...
def main():
    ...
    print(
        f"     Products  : {meta['total_products']:,}  ({meta['total_variants']:,} variants)"
    )
    print(
        f"     Customers : {meta['total_customers']:,}  ({meta['credit_customers']:,} credit)"
    )
