...
// --- PRINT PREVIEW COMPONENT (always white — print styling) ---
    ...
    const SaleRow = (index, sale) => {
        ...
        return (
            <div
                ...
                className={`border-b border-border hover:bg-muted/40 cursor-pointer transition-colors group px-4 py-2.5 text-sm ${SALES_GRID}`}
            >
                <div className="font-medium text-blue-600">#{sale.id}</div>
                <div className="truncate min-w-0">
                    <span className="text-foreground font-medium block truncate">{sale.items?.map(i => i.variant_name).join(', ')}</span>
                    <span className="text-[11px] text-muted-foreground block mt-0.5">
                        {new Date(sale.date).toLocaleDateString()}
                    </span>
                </div>
                <div className="text-right font-bold text-foreground">₹{totalAmount.toFixed(2)}</div>
                <div className="text-right flex justify-end">
                    <Button
                        variant="ghost"
                        size="icon"
                        onClick={(e) => { e.stopPropagation(); setDeleteItem({ type: 'sale', id: sale.id }); }}
                        className="h-7 w-7 text-muted-foreground/40 hover:text-red-600 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                        <Trash2 size={14} />
                    </Button>
                </div>
    ...
    const PaymentRow = (index, payment) => {
        ...
        return (
            <div
                ...
                className={`border-b border-border hover:bg-muted/40 cursor-pointer transition-colors group px-4 py-2.5 text-sm ${PAYMENTS_GRID}`}
            >
                <div className="text-xs text-muted-foreground font-medium">
                    {new Date(payment.date).toLocaleDateString()}
                </div>
                <div>
                    <Badge variant="secondary" className="bg-green-500/10 text-green-700 dark:text-green-400 hover:bg-green-500/20 border-green-500/20 font-normal px-2 py-0.5 text-[10px]">
                        Collection
                    </Badge>
                </div>
                <div className="text-right font-bold text-green-600 dark:text-green-400">-₹{Number(payment.amount).toFixed(2)}</div>
                <div className="text-right flex justify-end">
                    <Button
                        variant="ghost"
                        size="icon"
            ...
            <div className="max-w-7xl mx-auto space-y-6 p-6 pb-20 print:hidden animate-in fade-in duration-500 h-screen flex flex-col overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-shrink-0">
                    <div className="flex items-center gap-4">
                        <Button variant="ghost" size="icon" onClick={() => navigate('/customers')} className="h-10 w-10 -ml-2 text-muted-foreground hover:text-foreground hover:bg-muted rounded-full">
                            <ArrowLeft className="h-5 w-5" />
                        </Button>
                        <div>
                            <h1 className="text-3xl font-bold tracking-tight text-foreground">{customer.name}</h1>
                            <div className="flex items-center gap-2 mt-1">
                                <Badge variant="secondary" className="bg-muted text-muted-foreground hover:bg-muted border-border font-mono text-[10px] px-2">ID #{customer.id}</Badge>
                            </div>
                        </div>
                    </div>
                    <div className="flex gap-3">
                        <Button variant="outline" onClick={() => setIsPreviewOpen(true)} className="gap-2 border-border text-foreground shadow-sm hover:bg-muted">
                            <Printer size={16} /> Statement
                        </Button>
                        <Button variant="outline" onClick={() => setDeleteItem({ type: 'customer' })} className="gap-2 border-red-200 text-red-600 bg-transparent shadow-sm hover:bg-red-500/10 hover:text-red-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-shrink-0">
                    <Card className="shadow-sm border-border bg-card">
                        <CardContent className="flex items-center gap-5 p-6">
                            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-500/10 text-blue-600 border border-blue-500/20">
                                <Phone className="h-6 w-6" />
                            </div>
                            <div>
                                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Mobile</p>
                                <p className="text-xl font-bold text-foreground mt-0.5">{customer.mobile || "N/A"}</p>
                            </div>
                        </CardContent>
                    </Card>
                    <Card className="shadow-sm border-border bg-card">
                        <CardContent className="flex items-center gap-5 p-6">
                            <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-purple-500/10 text-purple-600 border border-purple-500/20">
                                <MapPin className="h-6 w-6" />
                            </div>
                            <div>
                                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Address</p>
                                <p className="text-xl font-bold text-foreground mt-0.5 truncate max-w-[200px]" title={customer.address}>
                    ...
                    <Card className="shadow-sm border-border bg-card">
                        <CardContent className="flex items-center gap-5 p-6">
                            <div className={`flex h-12 w-12 items-center justify-center rounded-lg border ${customer.balance > 0 ? 'bg-red-500/10 text-red-600 border-red-500/20' : 'bg-green-500/10 text-green-600 border-green-500/20'}`}>
                                <Wallet className="h-6 w-6" />
                            </div>
                            <div>
                                <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-wider">Balance Due</p>
                                <p className={`text-2xl font-bold mt-0.5 ${customer.balance > 0 ? 'text-foreground' : 'text-green-600 dark:text-green-400'}`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
                    ...
                    {/* SALES LIST */}
                    <Card className="flex flex-col overflow-hidden shadow-sm border-border h-full bg-card">
                        <CardHeader className="px-6 py-4 border-b border-border bg-card flex flex-row items-center justify-between space-y-0 flex-shrink-0">
                            <CardTitle className="flex items-center gap-2 text-base font-bold text-foreground">
                                <ShoppingBag className="h-4 w-4 text-muted-foreground" /> Sales History
                            </CardTitle>
                            <Badge variant="secondary" className="bg-muted text-muted-foreground font-mono">{sales.length} Records</Badge>
                        </CardHeader>

                        <div className={`bg-muted/30 border-b border-border px-4 py-2 text-[11px] font-bold text-muted-foreground uppercase tracking-wider flex-shrink-0 ${SALES_GRID}`}>
                            <div>ID</div>
                            <div>Items</div>
                        ...
                        <CardContent className="p-0 flex-1 min-h-0 bg-card">
                            {sales.length === 0 ? (
                                ...
                                <Virtuoso
                                    ...
                                    components={{
                                        Footer: () => loadingSales ? <div className="p-2 text-center text-xs text-muted-foreground border-t border-border"><Loader2 className="animate-spin h-3 w-3 inline mr-2" />Loading...</div> : null
                                    }}
                                />
                            )}
                        </CardContent>
                    ...
                    <Card className="flex flex-col overflow-hidden shadow-sm border-border h-full bg-card">
                        <CardHeader className="px-6 py-4 border-b border-border bg-card flex flex-row items-center justify-between space-y-0 flex-shrink-0">
                            ...
                            <Badge variant="secondary" className="bg-muted text-muted-foreground font-mono">{payments.length} Records</Badge>
                        </CardHeader>

                        <div className={`bg-muted/30 border-b border-border px-4 py-2 text-[11px] font-bold text-muted-foreground uppercase tracking-wider flex-shrink-0 ${PAYMENTS_GRID}`}>
                            <div>Date</div>
                            <div>Type</div>
                        ...
                        <CardContent className="p-0 flex-1 min-h-0 bg-card">
                            {payments.length === 0 ? (
                                ...
                                <Virtuoso
                                    ...
                                    components={{
                                        Footer: () => loadingPayments ? <div className="p-2 text-center text-xs text-muted-foreground border-t border-border"><Loader2 className="animate-spin h-3 w-3 inline mr-2" />Loading...</div> : null
                                    }}
                                />
                            )}
                        </CardContent>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-muted/20">
                            <div className="space-y-3">
                                {selectedSale?.items.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center p-4 bg-card rounded-lg border border-border shadow-sm">
                                        <div>
                                            <p className="font-bold text-sm text-foreground">{item.variant_name}</p>
                                            <p className="text-xs text-muted-foreground mt-1 font-medium">Qty: {item.quantity} × ₹{item.price_at_sale}</p>
                                        </div>
                                        <p className="font-bold text-foreground">₹{(item.quantity * item.price_at_sale).toFixed(2)}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        <div className="p-6 border-t border-border bg-card flex justify-between items-center">
                            <span className="font-bold text-muted-foreground uppercase text-xs tracking-wider">Total Sale Value</span>
                    ...
                    <DialogContent className="sm:max-w-sm">
                        ...
                        <div className="py-6 space-y-4">
                            <div className="flex justify-between items-center p-3 rounded-lg border border-border bg-muted/20">
                                ...
                                <span className="font-bold text-foreground">{selectedPayment && new Date(selectedPayment.payment_date).toLocaleDateString()}</span>
                            </div>
                            <div className="flex justify-between items-center p-4 bg-green-500/10 border border-green-500/20 rounded-lg">
                                <span className="text-green-700 dark:text-green-400 font-bold uppercase text-xs tracking-wider">Amount Paid</span>
                                <span className="font-bold text-3xl text-green-700 dark:text-green-400">₹{selectedPayment?.amount.toFixed(2)}</span>
                            </div>
                        </div>
                        <DialogFooter>
