...
export default function SupplierDetailsPage() {
  ...
  const InvoiceRow = ({ index, data }) => {
    ...
    return (
      <div 
        ...
        className={`
          flex items-center justify-between px-6 py-3 border-b border-slate-100 cursor-pointer transition-colors
          ${isHighlighted 
            ? 'bg-blue-50 z-10' 
      ...
      >
        <div className="flex-1 min-w-0 pr-4">
            <div className="flex items-center gap-3 mb-1">
                <span className={`font-mono text-xs px-1.5 py-0.5 rounded border ${isHighlighted ? 'bg-blue-100 text-blue-700 border-blue-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                    #{inv.id}
                </span>
        ...
        <div className="text-right flex flex-col items-end gap-0.5">
            <div className={`font-bold font-mono ${isHighlighted ? 'text-blue-900' : 'text-slate-900'}`}>₹{total.toFixed(2)}</div>
            <Badge variant="outline" className="text-[10px] h-5 px-1.5 text-slate-500 font-normal border-slate-200">
               {inv.itemcount || 0} items
            </Badge>
        </div>
        
        <div className="w-8 flex justify-end">
             <ChevronRight size={16} className={`transition-opacity ${isHighlighted ? 'opacity-100 text-blue-500' : 'opacity-0 group-hover:opacity-50 text-slate-300'}`}/>
        </div>
      </div>
    );
  };

  if (loading) return <div className="h-screen flex items-center justify-center space-y-4 flex-col"><Loader2 className="animate-spin text-blue-600 h-10 w-10"/><p className="text-slate-500 font-medium">Loading Supplier Profile...</p></div>;
  ...
  return (
    <>
      ...
      {/* FIX 1: Enforced h-screen to give children a reference height. Overflow-hidden handles scroll inside list. */}
      <div className="max-w-7xl mx-auto space-y-6 p-6 pb-6 print:hidden animate-in fade-in duration-500 h-screen flex flex-col overflow-hidden">
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 flex-shrink-0">
            <div className="flex items-center gap-3">
                <Button variant="ghost" size="icon" onClick={() => navigate('/purchase-ledger')} className="h-10 w-10 rounded-full hover:bg-slate-100 -ml-2 text-slate-500">
                    <ArrowLeft size={20}/>
                </Button>
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-slate-900">{supplier?.name}</h1>
                    <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-sm text-slate-500">Supplier Profile & History</span>
                        <Badge variant="secondary" className="font-mono text-[10px] text-slate-500 bg-slate-100 border-slate-200">ID #{supplier?.id}</Badge>
                    </div>
                </div>
            </div>
         </div>
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-shrink-0">
            <Card className="border-slate-200 shadow-sm">
                <CardContent className="flex items-center gap-4 p-5">
                    <div className="p-0 text-slate-400"><Phone size={24} strokeWidth={1.5}/></div>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Mobile Contact</p>
                        <p className="font-semibold text-slate-900 mt-0.5">{supplier?.mobile || "N/A"}</p>
                    </div>
                </CardContent>
            </Card>
            <Card className="border-slate-200 shadow-sm">
                <CardContent className="flex items-center gap-4 p-5">
                    <div className="p-0 text-slate-400"><MapPin size={24} strokeWidth={1.5}/></div>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Address</p>
                        <p className="font-semibold text-slate-900 mt-0.5 truncate max-w-[200px]" title={supplier?.address}>{supplier?.address || "N/A"}</p>
                    </div>
                </CardContent>
            </Card>
            <Card className="border-slate-200 shadow-sm">
                <CardContent className="flex items-center gap-4 p-5">
                    <div className="p-0 text-slate-400"><Truck size={24} strokeWidth={1.5}/></div>
                    <div>
                        <p className="text-xs font-bold uppercase tracking-wider text-slate-500">Total Volume</p>
                        <p className="text-xl font-bold text-slate-900 tracking-tight mt-0.5">₹{history.reduce((sum, h) => sum + (h.totalamount || h.total_amount || 0), 0).toFixed(2)}</p>
                    </div>
                </CardContent>
            </Card>
         </div>

         {/* FIX 2: flex-1 + min-h-0 allows the card to take ALL remaining space but allows shrinking. */}
         <Card className="flex-1 border-slate-200 shadow-sm flex flex-col overflow-hidden min-h-0">
            <CardHeader className="py-4 px-6 border-b border-slate-100 bg-white flex flex-row items-center justify-between space-y-0 flex-shrink-0">
                <div className="flex items-center gap-2">
                    <FileText className="text-slate-400" size={18}/>
                    <CardTitle className="text-base font-bold text-slate-800">Purchase History</CardTitle>
                    <Badge variant="secondary" className="ml-2 bg-slate-100 text-slate-600 border-slate-200">{filteredHistory.length}</Badge>
                </div>
            ...
            {/* FIX 3: Flex container for content to ensure Virtuoso parent has dimension */}
            <CardContent className="p-0 flex flex-col flex-1 bg-white min-h-0">
                {filteredHistory.length === 0 ? (
                   <div className="flex flex-col items-center justify-center h-full py-20 text-slate-400 gap-3">
                      <div className="bg-slate-50 p-4 rounded-full"><Search size={24} className="opacity-50" /></div>
                      <p className="text-sm font-medium">No invoices match your search.</p>
                   </div>
                ) : (
                   <div className="flex flex-col flex-1 min-h-0">
                       <div className="grid grid-cols-[1fr_auto_32px] px-6 py-2 bg-slate-50/50 border-b border-slate-100 text-xs font-bold uppercase tracking-wider text-slate-500 flex-shrink-0">
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col p-0 overflow-hidden">
            <DialogHeader className="p-4 border-b border-slate-100 flex-shrink-0"><DialogTitle>Print Preview</DialogTitle></DialogHeader>
            <div className="flex-1 overflow-y-auto bg-slate-100 p-8 flex justify-center">
                <div className="shadow-xl bg-white scale-90 origin-top">
                    {previewData && <PrintInvoiceContent invoice={previewData.invoice} details={previewData.details} supplier={supplier} storeConfig={storeConfig} />}
                </div>
            </div>
            <DialogFooter className="p-4 border-t border-slate-100 bg-white flex-shrink-0">
